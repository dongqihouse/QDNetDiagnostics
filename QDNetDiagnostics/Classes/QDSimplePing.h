...
@protocol QDSimplePingDelegate;

/*! Controls the IP address version used by QDSimplePing instances.
 */

typedef NS_ENUM(NSInteger, QDSimplePingAddressStyle) {
    QDSimplePingAddressStyleAny,          ///< Use the first IPv4 or IPv6 address found; the default.
    QDSimplePingAddressStyleICMPv4,       ///< Use the first IPv4 address found.
    QDSimplePingAddressStyleICMPv6        ///< Use the first IPv6 address found.
};
...
@property (nonatomic, weak, readwrite, nullable) id<QDSimplePingDelegate> delegate;

/*! Controls the IP address version used by the object.
 *  \details You should set this value before starting the object.
 */

@property (nonatomic, assign, readwrite) QDSimplePingAddressStyle addressStyle;
...
/*! A delegate protocol for the QDSimplePing class.
 */

@protocol QDSimplePingDelegate <NSObject>

@optional

/*! A QDSimplePing delegate callback, called once the object has started up.
...
/*! A QDSimplePing delegate callback, called if the object fails to start up.
 *  \details This is called shortly after you start the object to tell you that the 
...
/*! A QDSimplePing delegate callback, called when the object has successfully sent a ping packet. 
...
- (void)simplePing:(QDSimplePing *)pinger didSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber;

/*! A QDSimplePing delegate callback, called when the object fails to send a ping packet. 
 *  \details Each call to `-sendPingWithData:` will result in either a 
 *      `-simplePing:didSendPacket:sequenceNumber:` delegate callback or a 
 *      `-simplePing:didFailToSendPacket:sequenceNumber:error:` delegate callback (unless you 
 *      stop the object before you get the callback).  These callbacks are currently delivered 
...
- (void)simplePing:(QDSimplePing *)pinger didFailToSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber error:(NSError *)error;

/*! A QDSimplePing delegate callback, called when the object receives a ping response.
...
/*! A QDSimplePing delegate callback, called when the object receives an unmatched ICMP message.
 ...
 *  
 *      This is declared in the header because clients of QDSimplePing might want to use 
